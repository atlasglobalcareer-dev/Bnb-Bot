"""Production scanner configuration for BNB high-confidence micro-cap alerts."""
import os
from dataclasses import dataclass, field
from dotenv import load_dotenv

load_dotenv()
MAX_TARGET_MARKET_CAP_USD = 55_000.0
...
class Settings:
    ...
    min_market_cap_usd: float = field(default_factory=lambda: _float("MIN_MARKET_CAP_USD", 20_000))
    ...
    min_liquidity_usd: float = field(default_factory=lambda: _float("MIN_LIQUIDITY_USD", 15_000))
    min_volume_24h_usd: float = field(default_factory=lambda: _float("MIN_VOLUME_24H_USD", 20_000))
    min_score_to_alert: float = field(default_factory=lambda: _float("MIN_SCORE_TO_ALERT", 70))
    min_1h_transactions: int = field(default_factory=lambda: _int("MIN_1H_TRANSACTIONS", 20))
    min_pool_age_minutes: int = field(default_factory=lambda: _int("MIN_POOL_AGE_MINUTES", 60))
    max_pool_age_minutes: int = field(default_factory=lambda: _int("MAX_POOL_AGE_MINUTES", 4320))
    min_buy_sell_ratio: float = field(default_factory=lambda: _float("MIN_BUY_SELL_RATIO", 1.5))
    min_liquidity_to_mc_ratio: float = field(default_factory=lambda: _float("MIN_LIQUIDITY_MC_RATIO", 0.30))
    ...
    def validate(self):
        ...
        if self.max_market_cap_usd > MAX_TARGET_MARKET_CAP_USD:
            problems.append("MAX_MARKET_CAP_USD cannot exceed $55,000")
        return problems
