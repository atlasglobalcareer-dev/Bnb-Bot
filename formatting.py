"""Telegram alert formatting with full screening data and audit status."""
...
def format_alert(pool: PoolData, score: ScoreBreakdown) -> str:
    lines = [
        ...
        f"🟢 Buys 6h / Sells 6h: {pool.buys_6h} / {pool.sells_6h}",
    ...
    # Audit state is kept on the score object by the scoring layer when available.
    hp_checked = getattr(score, "honeypot_checked", False)
    hp_value = getattr(score, "is_honeypot", None)
    buy_tax = getattr(score, "buy_tax", None)
    sell_tax = getattr(score, "sell_tax", None)
    if hp_checked:
        lines.append(f"🛡 Honeypot: {'YES — BLOCKED' if hp_value is True else 'NO — passed'}")
        if buy_tax is not None:
            lines.append(f"💸 Buy tax: {buy_tax:.2f}%")
        if sell_tax is not None:
            lines.append(f"💸 Sell tax: {sell_tax:.2f}%")
    else:
        lines.append("🛡 Honeypot: UNKNOWN / check unavailable")

    contract_available = getattr(score, "contract_available", False)
    verified = getattr(score, "contract_verified", None)
    holder_pct = getattr(score, "top10_holder_pct", None)
    owner_renounced = getattr(score, "owner_renounced", None)
    ...
    lines += [
        "",
        f"[View chart]({pool.dex_url})",
        "",
        "_Research/screening only — not financial advice. Meme tokens are extremely high risk._",
    ]
    return "\n".join(lines)
