...
class Scanner:
    ...
    async def _enrich_market_caps(self, pools):
        initial_missing = sum(1 for p in pools if p.market_cap_usd <= 0); gecko_enriched = dex_pair_enriched = dex_search_enriched = still_missing = 0
        for pool in pools:
            ...
            try:
                ...
                if mc > 0: pool.market_cap_usd = mc; dex_pair_enriched += 1; continue
            ...
            if pool.token_address:
                try:
                    ...
                    if usable: pool.market_cap_usd = float(usable[0]["marketCap"]); dex_search_enriched += 1; continue
    ...
    async def run_once(self):
        ...
        for pool in pools:
            ...
            # Security is now a hard gate: unknown/unverified/high-tax/risky ownership never alerts.
        ...
        logger.info("FILTER SUMMARY: candidates=%d missing_mc=%d below_min_mc=%d at_or_above_55k=%d too_new=%d too_old=%d low_liquidity=%d low_liquidity_mc_ratio=%d low_volume=%d low_transactions=%d sell_pressure=%d reached_scoring=%d blocked=%d unknown_honeypot=%d unverified_contract=%d bad_tax=%d bad_owner=%d bad_holders=%d low_score=%d cooldown=%d telegram_unconfigured=%d telegram_failed=%d alerts_sent=%d", len(pools), stats["mc_missing_or_zero"], stats["mc_below_min"], stats["mc_at_or_above_max"], stats["age_too_new"], stats["age_too_old"], stats["liquidity_below_floor"], stats["low_liquidity_mc_ratio"], stats["volume_below_floor"], stats["transactions_below_floor"], stats["sell_pressure"], stats["reached_scoring"], stats["blocked_by_audit"], stats["unknown_honeypot"], stats["unverified_contract"], stats["bad_tax"], stats["bad_owner"], stats["bad_holders"], stats["score_below_threshold"], stats["already_alerted_recently"], stats["telegram_unconfigured"], stats["telegram_delivery_failed"], stats["alerts_sent"])
